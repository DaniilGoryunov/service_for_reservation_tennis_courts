...
import psycopg2
import datetime
from services.reserv import cancel_reservation
from services.auth import DB_CONFIG
...
def display_reservations(reservations, role):
    ...
    if future_reservations:
        ...
        for reservation in future_reservations:
            ...
            if role == 'user': st.write(f" **Номер корта** {username}")
            else: st.write(f" **Пользователь** {username}")
